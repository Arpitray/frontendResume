...
import { matchResume, uploadJob, uploadResume } from "@/src/lib/api";
...
export default function MatchPage() {
    ...
    async function runMatch() {
        ...
        setLoading(true);
        try {
            const result = await matchResume(resumeId, currentJobId);
            setData(result);
        } catch (err) {
            console.error(err);
                ...
                {data && (
                    <div className="space-y-8">
                        ...
                        {data.ai_feedback && (() => {
                            ...
                            try {
                                if (typeof data.ai_feedback === 'string') {
                                    let str = data.ai_feedback.trim();
                                    // 1. Try extracting from markdown code block
                                    const jsonMatch = str.match(/```json\s*([\s\S]*?)\s*```/i);
                                    if (jsonMatch && jsonMatch[1]) {
                                        str = jsonMatch[1];
                                    } else {
                                        // 2. If no code block, try finding outermost JSON object (handles trailing text)
                                ...
                                return (
                                    <div className="mt-12 p-8 bg-card border border-border rounded-2xl shadow-lg">
                                        ...
                                        <div className="text-foreground/90 whitespace-pre-wrap leading-relaxed font-mono text-sm bg-muted/30 p-4 rounded-lg">
                                            {typeof data.ai_feedback === 'string' ? data.ai_feedback.replace(/```json/g, '').replace(/```/g, '') : "Feedback available but could not be parsed."}
                                        </div>
                                    </div>
                                );
                            }
                        ...
                        {data.learning_path && (() => {
                            let learningPath;
                            try {
                                if (typeof data.learning_path === 'string') {
                                    let str = data.learning_path.trim();
                                    const jsonMatch = str.match(/```json\s*([\s\S]*?)\s*```/i);
                                    if (jsonMatch && jsonMatch[1]) {
                                        str = jsonMatch[1];
                                    } else {
                            ...
                            const roadmapData = learningPath.roadmap || learningPath.phases || (Array.isArray(learningPath) ? learningPath : null);
